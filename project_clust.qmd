...
editor: visual
---
Comenzaremos recordando que el dataset se basa en observaciones sobre viviendas en Boston. El objetivo de este dataset era el de ver si las distintas variables eran buenas prediciendo el valor medio de la vivienda.
Primero haremos un análisis de componentes principales de nuestros datos y después haremos clustering para ver cómo se agrupan nuestros datos.
## ACP
...
De este gráfico es complicado sacar una información clara sobre el dataset pero al menos podemos observar la correlación de las variables. En general parece que hay muchas variables que se correlacionan entre ellas.
En el siguiente gráfico vemos qué variables están mejor representadas por el análisis de componentes.
```{r}
...
clara_clusters
```
Ahora usaremos el paquete clara
```{r,echo=FALSE}
fviz_cluster(object = clara_clusters, ellipse.type = "t", geom = "point") +
theme_bw() +
theme(legend.position = "none")
...
table(clara_clusters$cluster, df_medv[, "medv"],
 dnn = list("cluster", "grupo real"))
```
También hacemos el clustering  usando la correlación de Pearson y vemos que los clusters quedan muy parecidos a los anteriores, por lo tanto cualitativamente no hay mucha diferencia entre ambas distancias.
```{r,echo=FALSE,warnings=FALSE,message=FALSE}
set.seed(123)
dist_cor <- 1 - cor(x = df_numeric, method = "pearson")
mat_dist1 <- get_dist(x = df_numeric, method = "pearson")
mat_dist <- as.matrix(mat_dist1)

```

```{r,echo=FALSE}
fviz_cluster(object = pam_clusters, data = df_numeric, ellipse.type = "t", repel = TRUE,diss=mat_dist,geom="point") +
theme_bw()
```

Finalmente veremos cómo queda con un cluster jerárquico, que normalmente se representarían con dendogramas pero como el conjunto de datos es demasiado grande lo representaremos sin. La distancia también que usaremos será la de Pearson.
```{r,echo=FALSE,warnings=FALSE,message=FALSE}
mat_dist <- 1- cor(x = df_scaled2, method = "pearson")
mat_dist2 <- dist(x=df_scaled2, method = "manhattan")
# Dendrogramas con linkage complete y average
hc_ward2 <- hclust(d = mat_dist2, method = "ward.D2")
```

```{r,echo=FALSE}
fviz_cluster(object = list(data = df_scaled2, cluster = cutree(hc_ward2, k = 3)),
 ellipse.type = "convex",
 repel = TRUE,
 show.clust.cent = FALSE, geom='point') +
theme_bw()
...
```{r,echo=FALSE,warnings=FALSE}
table(cutree(hc_ward2, k = 3), df_medv[, "medv"],
 dnn = list("cluster", "grupo real"))
```
